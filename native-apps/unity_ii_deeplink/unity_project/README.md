...
5. The app constructs a `DelegationIdentity` from the chain and calls the backend canister directly using [ICP.NET](https://github.com/edjCase/ICP.NET).
...
- [**`GreetingClient.cs`**](./Assets/Scripts/GreetingClient.cs) — Auto-generated Candid client for the backend canister. Can be regenerated from `backend/backend.did` using the `ClientGenerator` in [ICP.NET](https://github.com/edjCase/ICP.NET).
...
Select the `AgentAndPlugin` GameObject in the scene hierarchy and set these fields on the `ICPAgent` component before building:
...
The ICP.NET DLLs in `Assets/Plugins/ICP.NET/` are from [edjCase/ICP.NET](https://github.com/edjCase/ICP.NET) v7.0.2, imported via `agent.unitypackage` from the [releases page](https://github.com/edjCase/ICP.NET/releases/tag/7.0.2). To upgrade, download the new `agent.unitypackage` and re-import it into Unity (Assets → Import Package → Custom Package).
