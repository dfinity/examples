...
# Extract the canister version of the most recent change from the history.
# Using the history itself avoids assuming how many steps icp deploy takes.
deploy_version=$(echo "$info_result" | grep 'canister_version' | tail -1 | grep -oE '[0-9]+' | head -1)
echo "Most recent change version: $deploy_version"
...
echo "=== Test 3: canister_controllers returns the controllers at the current version ==="
result=$(icp canister call backend canister_controllers "(principal \"$test_id\", variant { at_version = $deploy_version : nat64 })")
echo "$result"
...
result=$(icp canister call backend canister_module_hash "(principal \"$test_id\", variant { at_version = $deploy_version : nat64 })")
echo "$result"
echo "$result" | grep -q 'blob' && echo "PASS" || (echo "FAIL" && exit 1)

echo "=== Test 5: canister_deployment_chain shows the code_deployment entry ==="
result=$(icp canister call backend canister_deployment_chain "(principal \"$test_id\", variant { at_version = $deploy_version : nat64 })")
echo "$result"
...
echo "=== Test 8: canister_controllers at the old version does NOT include the added controller ==="
result=$(icp canister call backend canister_controllers "(principal \"$test_id\", variant { at_version = $deploy_version : nat64 })")
echo "$result"
