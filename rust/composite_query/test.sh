...
result=$(icp canister call caller put '(1 : nat, 1337 : nat)') && \
  echo "$result" && \
  echo "PASS" || (echo "FAIL" && exit 1)

echo "=== Test 2: put a second value for the same key returns the old value ==="
result=$(icp canister call caller put '(1 : nat, 42 : nat)') && \
  echo "$result" && \
  echo "$result" | grep -q '1_337' && \
  echo "PASS" || (echo "FAIL" && exit 1)

echo "=== Test 3: get (composite query) retrieves the stored value ==="
result=$(icp canister call --query caller get '(1 : nat)') && \
  echo "$result" && \
  echo "$result" | grep -q 'opt (42' && \
  echo "PASS" || (echo "FAIL" && exit 1)

echo "=== Test 4: get_update returns the same value via an update call ==="
result=$(icp canister call caller get_update '(1 : nat)') && \
  echo "$result" && \
  echo "$result" | grep -q 'opt (42' && \
  echo "PASS" || (echo "FAIL" && exit 1)

echo "=== Test 5: get returns null for a missing key ==="
result=$(icp canister call --query caller get '(99 : nat)') && \
  echo "$result" && \
  echo "$result" | grep -q 'null' && \
  echo "PASS" || (echo "FAIL" && exit 1)

echo "=== Test 6: composite query routes correctly across partitions ==="
icp canister call caller put '(2 : nat, 100 : nat)' && \
  icp canister call caller put '(3 : nat, 200 : nat)' && \
  icp canister call caller put '(4 : nat, 300 : nat)' && \
  result1=$(icp canister call --query caller get '(1 : nat)') && \
  result2=$(icp canister call --query caller get '(2 : nat)') && \
  result3=$(icp canister call --query caller get '(3 : nat)') && \
  result4=$(icp canister call --query caller get '(4 : nat)') && \
...
result=$(icp canister call --query caller lookup '(1 : nat)') && \
  echo "$result" && \
