<script>
  ...
  import { backend_dummy } from "../../../declarations/backend_dummy/index.js";
  ...
  async function getExchangeRates(start, end) {
    ...
    const rates = await backend_dummy.get_rates(start, end);
    var sortedRates = rates.sort(function(a, b) { return Number(a[0]) - Number(b[0]) })
    var next = start;
            ...
            x: {
              ticks: {
                callback: function(val, index) {
                  return index % 2 === 0 ? timestampToString(this.getLabelForValue(val)) : '';
                },
              },
            },
          }
...
{#if missingData}
<div class="warning">
  <Fa icon={faTriangleExclamation} /> Parts of the data are missing and are still being fetched. Retry a bit later...
</div>
{/if}
