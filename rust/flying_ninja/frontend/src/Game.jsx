...
const Game = () => {
  ...
  const fetchLeaderboard = async () => {
    try {
      const entries = await backend.get_leaderboard();
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        const randomness = await backend.get_randomness();
  ...
  const checkHighScore = async () => {
    const isHighScore = await backend.is_high_score(BigInt(scoreRef.current));
  ...
  const submitScore = async () => {
    if (playerName.trim() !== '') {
      await backend.add_leaderboard_entry(playerName, BigInt(scoreRef.current));
