...
static MIN_INTERVAL_SECS: std::sync::atomic::AtomicU64 =
...
fn heartbeat() {
    ...
    LAST_PERIODIC_TASK_TIME.with(|last_periodic_task_time| {
        let min_interval_secs = MIN_INTERVAL_SECS.load(std::sync::atomic::Ordering::Relaxed);
...
/// Sets a periodic task interval in seconds to increment the `COUNTER`.
#[ic_cdk::update]
fn set_interval_secs(secs: u64) {
    MIN_INTERVAL_SECS.store(secs, std::sync::atomic::Ordering::Relaxed);
