...
# Runs a canister call that is expected to trap and verifies the trap message.
# Usage: assert_traps <expected_substring> <canister_call_args...>
assert_traps() {
  local expected="$1"; shift
  trap_output=$(icp canister call "$@" 2>&1 >/dev/null || true)
  echo "$trap_output" | grep -q "$expected" && echo "(trap: $expected) PASS" || \
    (echo "FAIL: expected trap message '$expected', got: $trap_output" && exit 1)
}
...
icp canister call backend set_non_processed_items '(vec { "mint" })'
assert_traps "panicking callback!" backend process_single_item_with_panicking_callback '("mint", variant { TrueAsyncCall })'
result=$(icp canister call --query backend is_item_processed '("mint")') && \
  echo "$result" && \
...
icp canister call backend set_non_processed_items '(vec { "mint" })'
assert_traps "panicking callback!" backend process_single_item_with_panicking_callback '("mint", variant { FalseAsyncCall })'
result=$(icp canister call --query backend is_item_processed '("mint")') && \
  echo "$result" && \
  echo "$result" | grep -q 'false' && \
  echo "PASS" || (echo "FAIL" && exit 1)
...
assert_traps "panicking callback!" backend process_all_items_with_panicking_callback '("mint2", variant { TrueAsyncCall })'
