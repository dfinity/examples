...
*/
  public func subaccountForInvoice(invoiceId : Nat, invoiceCreator : Principal) : Blob {
    ...
    return Blob.fromArray(Array.flatten<Nat8>([crc32bytes, hashSum]));
  };
...
/**
* args : { p : Principal, subaccount : Blob } Note subaccount is not validated as this method is to be used in conjuction with subaccountForInvoice and subaccountForPrincipal
...
*/
  public func toAccountIdentifierAddress(p : Principal, subaccount : Blob) : Blob {
    ...
    return AccountIdentifierBlob.fromPrincipal(p, ?Blob.toArray(subaccount));
  };

/**
* args : { b : Blob }
* Takes a blob and returns it in text form
* For converting account identifiers or subaccounts into their human readable form
*/
  public func toHumanReadableForm(b : Blob) : Text {
    return Hex.encode(Blob.toArray(b));
  };
...
*/
  public func validateAccountIdentifier(accountIdentifier : Blob) : Bool {
    ...
    return Array.equal(Binary.BigEndian.fromNat32(crc32), checksumPart, Nat8.equal);
  };
...
*/
  public func accountIdentifierFromValidText(accountId : Text) : Result.Result<Blob, Text> {
    switch (AccountIdentifierBlob.fromText(accountId)) {
      case (#ok(aid)) {
        if (validateAccountIdentifier(aid)) {
          return #ok(aid);
        } else {
