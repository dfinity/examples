...
As stated earlier, an example of integration with only two tokens, one for ICP and one for ICRC1 mapped to the variant tags `#ICP` and `#ICRC1` respectively, along with two corresponding mock ledgers that can be used, can be found in the `examples` directory and is the [motoko-seller-client](./examples/motoko-seller-client/) project.
