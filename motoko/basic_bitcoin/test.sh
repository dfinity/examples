...
echo "$result"
echo "$result" | grep -q '"' && echo "PASS" || (echo "FAIL" && exit 1)
...
echo "$result"
echo "$result" | grep -qE '[1-9]' && echo "PASS" || (echo "FAIL" && exit 1)

echo "=== Test 6: get_utxos returns synced chain state after mining ==="
addr=$(icp canister call backend get_p2pkh_address '()' | grep -o '"[^"]*"' | tr -d '"')
result=$(icp canister call backend get_utxos "(\"$addr\")")
echo "$result"
echo "$result" | grep -q 'tip_height = 101' && echo "PASS" || (echo "FAIL" && exit 1)
