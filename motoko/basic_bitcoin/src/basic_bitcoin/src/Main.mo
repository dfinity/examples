...
actor class BasicBitcoin(_network : Types.Network) {
  ...
  public func get_p2tr_raw_key_spend_address() : async BitcoinAddress {
    await BitcoinWallet.get_p2tr_raw_key_spend_address(NETWORK, KEY_NAME, DERIVATION_PATH);
  };

  public func send_from_p2tr_raw_key_spend_address(request : SendRequest) : async Text {
    Utils.bytesToText(await BitcoinWallet.send_p2tr_raw_key_spend(NETWORK, DERIVATION_PATH, KEY_NAME, request.destination_address, request.amount_in_satoshi));
  };
};
