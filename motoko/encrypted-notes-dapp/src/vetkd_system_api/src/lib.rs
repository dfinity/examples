...
async fn vetkd_public_key(request: VetKDPublicKeyRequest) -> VetKDPublicKeyReply {
    ...
    let derivation_path = {
        let paths: Vec<&[u8]> = request.derivation_path.iter().map(Vec::as_slice).collect();
        match request.canister_id {
            Some(canister_id) => DerivationPath::new(canister_id.as_slice(), &paths),
            None => DerivationPath::new(ic_cdk::caller().as_slice(), &paths),
        }
    };
...
async fn vetkd_encrypted_key(request: VetKDEncryptedKeyRequest) -> VetKDEncryptedKeyReply {
    ...
    let derivation_path = DerivationPath::new2(
