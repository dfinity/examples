...
actor class ExchangeRate() = this {
    ...
    let static FETCHED: HashMap<Timestamp, Rate>  = HashMap::new();
    let static REQUESTED: HashMap<Timestamp, Int> = HashMap::new();
    let static RATE_COUNTER: Nat = 0;
    ...
    as HTTP update call.
    */
    public shared(msg) func get_rates(range: TimeRange) -> RatesWithInterval: async () {
        ...
        let fetched: HashMap<Timestamp, Rate> = HashMap::new();
        ...
        for requested_min in start_min..end_min {
            ...
            switch FETCHED.get(&requested) {
                case None
                    ...
                    fetched.put(requested, map.get(&requested).unwrap().clone());
                    break;
                default
    ...
    public func sample_with_interval(fetched: HashMap<Timestamp, Rate>) -> RatesWithInterval {
    ...
    public func add_job_to_job_set(job: Timestamp) -> () {
    ...
    */
    public shared (msg) func get_rate(job: Timestamp): async() {
        let start_timestamp: Timestamp = job;
        let end_timestamp: Timestamp = job + REMOTE_FETCH_GRANULARITY * DATA_POINTS_PER_API;

        let host: Text = "api.pro.coinbase.com";
        // prepare system http_request call
        let request_headers = [
            { name: "Host".to_string(), value: host + ":443" },
            { name: "User-Agent".to_string(), value: "exchange_rate_canister".to_string() },
        ];
