...
`Map.mo` imports a Motoko actor class `Bucket(i, n)` from the library `Buckets.mo`. It also imports `mo:core/Cycles`, naming it `Cycles` for short, to share its cycles amongst the buckets it creates.
