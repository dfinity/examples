  ...
  test(
    ...
    async () => {
      ...
      expect(await Promise.all([
        ...
        tokenB(alice).icrc1_balance_of({ owner: alice.getPrincipal(), subaccount: [] }),
        tokenA(bob).icrc1_balance_of({ owner: bob.getPrincipal(), subaccount: [] }),
        tokenB(bob).icrc1_balance_of({ owner: bob.getPrincipal(), subaccount: [] }),
      ])).toEqual([
        100_020_000n,
        0n,
        0n,
      ...
      var tokenABalances: Record<string, bigint> = Object.fromEntries(balances[0]);
      var tokenBBalances: Record<string, bigint> = Object.fromEntries(balances[1]);
