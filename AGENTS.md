...
- **Deployable in Ninja's no-terminal environment.** No custom build scripts beyond the standard recipe build; no controller-only setup step a Ninja user can't perform (e.g. an `authorize` call that only a controller can make — such an example is excluded even if it is a single canister).
